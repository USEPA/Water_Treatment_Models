...
class CollocTestSuite(unittest.TestCase):
    ...
    def test_Br(self):
        _, _, _, Br, _ = hsdmix.colloc.build_collocation(3, 4)
    ...
    def test_Wr(self):
        _, _, _, _, Wr = hsdmix.colloc.build_collocation(3, 4)
        Wr_test = np.array([ 0.09490592,  0.19080837,  0.04761905])   
        assert np.allclose(Wr_test, Wr)


    def test_Az(self):
        _, Az, _, _, _ = hsdmix.colloc.build_collocation(3, 4)
