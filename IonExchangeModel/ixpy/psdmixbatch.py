...
class PSDMIXbatch:
    ...
    def solve(self, t_eval=None):
        ...
        EPOR = np.float64(self.params['EPOR']) # pellet porosity      
        ...
        Dp = np.float64(self.params['Dp']) # pore resin phase diffusion coefficient (cm**2/s)
...
#           TODO: Test if alternate quadratic form can avoid this
        ...
        def diffun(T, u):
            ...
            dY_dT = np.zeros([nr, NION])
            dY_dT_w = np.zeros([NION])
            
            #### Liquid phase concentrations
            du_dT[LIQUID, :] = a * J / VL
        
            # diffusion in bead  
            Y_swap = np.swapaxes(Y, 0, 1).reshape(nr*NION)
            Br_Y = Br_block.dot(Y_swap)
            Br_Y = Br_Y.reshape([NION, nr])
            Br_Y = np.swapaxes(Br_Y, 0, 1)
           
            Cpore_swap = np.swapaxes(Cpore, 0, 1).reshape(nr*NION)
            Br_Cpore = Br_block.dot(Cpore_swap)
            Br_Cpore = Br_Cpore.reshape([NION, nr])
            Br_Cpore = np.swapaxes(Br_Cpore, 0, 1)
            
            dY_dT = (EPOR * (Dp - Ds) * Br_Cpore + Ds * Br_Y) / rb**2  

            # intermediate term for dq_dT at bead surface        
            dY_dT_swap = np.swapaxes(dY_dT[:SURF, :],0,1).reshape(((nr-1)*NION))
            dY_dT_w = Wr1_block.dot(dY_dT_swap)
            dY_dT_w = dY_dT_w.reshape(NION)
            
            du_dT[RESIN:, :] = dY_dT
            du_dT[SURF, :] = (- J / rb - dY_dT_w) / Wr[-1]      
