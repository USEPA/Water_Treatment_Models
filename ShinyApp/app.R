...
ui <- fluidPage(theme=shinytheme("united"),
                ...
                sidebarLayout(
                  sidebarPanel(
                    ...
                    #actionButton("clear1", "clear data"),
                    textOutput("reject"),
                               ...
                               tabPanel("Input",
                                        ...
                                        tabsetPanel(
                                          tabPanel("Parameters",
                                                   ...
                                                   fluidRow(
                                                     ...
                                                     column(3,
                                                            ...
                                                            numericInput("Qv", "", 1400, min=0, max=1000000),
                                          ...
                                          tabPanel("Ions",
                                                   ...
                                                   br(), br(),
                                                   #dataInputUI("input-2"),
                                                   #dataOutputUI("output-2"),
                                                   h3("Concentration Points"),
                                                   dataEditUI("edit-2"),
                                                   br(), br()
                                                   ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$file1, {
   write.csv(paramdat(), "paramsheet.csv", row.names=FALSE)
  })

  observeEvent(input$file1, {
    write.csv(iondat$dat, "ionsheet.csv", row.names=FALSE)
  })

  observeEvent(input$file1, {
    write.csv(cindat(), "cinsheet.csv", row.names=FALSE)
  })
  
  
  data_edit <- dataEditServer("edit-1", data = "ionsheet.csv")
  dataOutputServer("output-1", data = data_edit)
  
  data_edit2<-dataEditServer("edit-2", data="cinsheet.csv")
  dataOutputServer("output-2", data=data_edit2)
  ...
  observeEvent(input$run_button, {
    out(HSDMIX_solve(paramdataframe(), data_edit(), data_edit2(), timeconverter(), nt_report))})
  ...
  allchemicals<-eventReactive(input$run_button, {for (x in 1:nrow(data_edit())){

    conc<-out()[[2]][, liquid_id(), x, outlet_id()]
    allchemicalconcs[[x]]<-conc
  }
    allconcdf<-data.frame(allchemicalconcs)
    colnames(allconcdf)<-data_edit()$name
    allconcdf
  })

  massvector<-reactive({c(data_edit()$mw/data_edit()$valence)})
