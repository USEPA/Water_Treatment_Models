...
server <- function(input, output, session) {
  ...
  cinrow<-eventReactive(input$add, {
    cinframe<-data.frame(
      input$name, input$mw, input$KxA, input$valence, input$kL, input$Ds
    )
    cinframe
  })
  
  cindataframe<-eventReactive(input$add, {
    newcindataframe<-rbind(cin2(), cinrow())
    newcindataframe
  })
  
  #output$IonsTable<-eventReactive(input$add, {renderDataTable(cindataframe())})
  ...
  output$sum<-eventReactive(input$apply_button, {renderDataTable(newdataframe())})
  ...
  HSDMIX_solve <- function (params, ions, Cin, nt_report){
   ...
   out <- reactive({
     HSDMIX_solve(newdataframe(), ion2(), cin2(), nt_report)
   })
